...
class HttpServer:
        ...
        def http_get(self,object_address,headers):
                ...
                if (object_address == '/list'): #tampilin list
                        nama_files = [os.path.basename(f) for f in files if os.path.isfile(f)]
                        filelist = '\n'.join(nama_files)
                        return self.response(200, 'OK', filelist, {'Content-type': 'text/plain'})
                if object_address.startswith('/delete/'): #hapus file
                        filename = object_address[len('/delete/'):]
                        target_path = os.path.join(thedir, filename)
                        if target_path not in files:
                                return self.response(404, 'Not Found', 'File gak ada.', {'Content-type': 'text/plain'})
                        try:
        ...
        def http_post(self,object_address,headers):
                if object_address == '/upload': #upload file
                        body = headers[-1] if headers else ''
                        filename = f'upload_{uuid.uuid4().hex}.txt'
                        with open(filename, 'w') as f:
                                f.write(body)
                        return self.response(200, 'OK', f'Uploaded as {filename}', {'Content-type': 'text/plain'})
                else:
                        headers ={}
                        isi = "kosong"
                        return self.response(200,'OK',isi,headers)
